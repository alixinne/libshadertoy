...
int parseOptions(string &shaderId, string &shaderApiKey, bool &dump, int argc,
                 char *argv[]) {
  ...
  desc.add_options()("help,h", po::bool_switch(&help)->default_value(false),
      ...
      "api,a", po::value<string>(&shaderApiKey)->required(),
...
int loadRemote(const string &shaderId, const string &shaderApiKey,
  ...
  int code = 0;
  try {
    string endpoint = string("https://www.shadertoy.com/api/v1/shaders/") +
                      shaderId + string("?key=") + shaderApiKey;
    ...
    for (int i = 0; i < shaderSpec["Shader"]["renderpass"].size(); ++i) {
      ...
      for (int j = 0; j < pass["inputs"].size(); ++j) {
        ...
        if (input["ctype"].compare("texture") == 0 ||
          ...
          string url =
              string("https://www.shadertoy.com") + input["src"].asString();
          ...
          BOOST_LOG_TRIVIAL(debug)
              ...
              << ": binding " << conf.source << " buffer " << conf.enabled();
        } else {
          stringstream ss;
    ...
    pair<string, shadertoy::BufferConfig> imagebuf(
        *contextConfig.bufferConfigs.begin());
...
int render(GLFWwindow *window, shadertoy::ContextConfig &contextConfig) {
  ...
  if (code == 0) {
    ...
    while (!glfwWindowShouldClose(window)) {
      ...
      state.V<shadertoy::iDate>() =
          glm::vec4(dt.date().year() - 1, dt.date().month(), dt.date().day(),
      ...
      if (btnstate == GLFW_PRESS) {
        ...
        state.V<shadertoy::iMouse>()[0] = state.V<shadertoy::iMouse>()[2] =
            xpos;
        state.V<shadertoy::iMouse>()[1] = state.V<shadertoy::iMouse>()[3] =
            contextConfig.height - ypos;
      } else {
      ...
      if (buffer) {
        ...
        std::cerr << "frame time: " << renderTime
                  ...
                  << " mpx/s: " << (contextConfig.width * contextConfig.height /
                                    (renderTime / 1e3))
                  << std::endl;
      }
...
int performRender(shadertoy::ContextConfig &contextConfig) {
  ...
  // Initialize window
  GLFWwindow *window =
      glfwCreateWindow(contextConfig.width, contextConfig.height,
                       "libshadertoy example 50-shadertoy", nullptr, nullptr);
