...
backends::gx::texture *basic_input::use()
{
	// Load if needed
	load();
...
backends::gx::texture *basic_input::bind(GLuint unit)
{
	sampler_->bind(unit);
	auto tex(use());

	// Check that we have a texture object
	error_assert(tex != nullptr, "Failed to get texture to bind to texture unit {} for input {}",
				 unit, static_cast<const void *>(this));
...
backends::gx::texture *basic_input::bind_image(GLuint unit)
{
	...
	// Check that we have a texture object
	error_assert(tex != nullptr, "Failed to get texture to bind to image unit {} for input {}",
				 unit, static_cast<const void *>(this));
	...
	tex->bind_image(unit, binding_.level, binding_.layered, binding_.layer, binding_.access,
					binding_.format);
	return tex;
}
