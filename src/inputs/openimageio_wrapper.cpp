...
void st_oiio_load_image_to_texture(const std::string &filename, bool vflip, GLenum &output_format,
								   const shadertoy::gl::texture &texture, int &width, int &height)
{
...
#define _TO_INTERNAL_FORMAT(suffix) (channels >= 4 ? GL_RGBA ## suffix : (channels >= 3 ? GL_RGB ## suffix : (channels >= 2 ? GL_RG ## suffix : (GL_R ## suffix))))
#define _TO_INPUT_FORMAT() (channels >= 4 ? GL_RGBA : (channels >= 3 ? GL_RGB : (channels >= 2 ? GL_RG : (GL_RED))))
	...
	if (vflip)
	{
		...
		image->read_image(decode_type,
					   buffer.data() + (height - 1) * scanlinesize,
					   AutoStride,
					   -scanlinesize,
					   AutoStride);
	}
