...
shared_ptr<gl::texture>
texture_engine::SOILTextureHandler(const InputConfig &inputConfig,
  ...
  if (ext.compare(".jpg") == 0 || ext.compare(".jpeg") == 0) {
    ...
    if ((infile = fopen(sp.c_str(), "rb")) == NULL) {
      BOOST_LOG_TRIVIAL(error) << "could not open " << texPath
                               << " for reading for input " << inputConfig.id;
    } else {
      ...
      if (decode) {
        ...
        texture->image_2d(GL_TEXTURE_2D, 0, GL_RGBA32F, cinfo.output_width,
                          cinfo.output_height, 0, fmt, GL_UNSIGNED_BYTE,
                          imgbuf);
    ...
    GLuint texid = SOIL_load_OGL_texture(
        inputConfig.source.c_str(), SOIL_LOAD_AUTO, GLuint(*texture),
        inputConfig.vflip ? SOIL_FLAG_INVERT_Y : 0);
    if (texid == 0) {
      BOOST_LOG_TRIVIAL(warning) << "failed to load '" << inputConfig.source
...
shared_ptr<gl::texture>
texture_engine::NoiseTextureHandler(const InputConfig &inputConfig,
                                    bool &skipTextureOptions, bool &skipCache,
                                    bool &framebufferSized) {
  // A noise texture
  ...
  // Load it
  noiseTexture->image_2d(GL_TEXTURE_2D, 0, GL_RED, config.width, config.height,
...
shared_ptr<gl::texture>
texture_engine::CheckerTextureHandler(const InputConfig &inputConfig,
                                      bool &skipTextureOptions, bool &skipCache,
                                      bool &framebufferSized) {
  ...
  for (int i = 0; i < config.width; ++i)
    for (int j = 1; j < config.height; ++j)
      chk[j * config.height + i] =
          ((i / size) % 2 == 0) ^ ((j / size) % 2 == 0) ? 255 : 0;

  // Load it
  checkerTexture->image_2d(GL_TEXTURE_2D, 0, GL_RED, config.width,
                           config.height, 0, GL_RED, GL_UNSIGNED_BYTE,
                           chk.data());

  BOOST_LOG_TRIVIAL(warning) << "generated " << size << "x" << size
                             << " checker texture for input " << inputConfig.id;
...
texture_engine::texture_engine(context_config &config) : config(config) {
  RegisterHandler("texture", bind(&texture_engine::SOILTextureHandler, this, _1,
                                  _2, _3, _4));
  RegisterHandler("noise", bind(&texture_engine::NoiseTextureHandler, this, _1,
                                _2, _3, _4));
  RegisterHandler("checker", bind(&texture_engine::CheckerTextureHandler, this,
                                  _1, _2, _3, _4));
}

void texture_engine::Initialize() {
  // Prepare the empty texture, a nice magenta checkerboard
  if (emptyTexture)
    return;

  emptyTexture = make_shared<gl::texture>(GL_TEXTURE_2D);
  ...
  emptyTexture->image_2d(GL_TEXTURE_2D, 0, GL_RED, width, height, 0, GL_RED,
                         GL_UNSIGNED_BYTE, chk.data());
}

void texture_engine::ClearState(bool framebufferSizeChange) {
...
gl::texture &texture_engine::GetInputTexture(const InputConfig &inputConfig) {
  ...
  if (it == inputTextures.end()) {
    ...
    if (handlerIt != handlers.end()) {
      bool skipTextureOptions = false, skipCache = false,
           framebufferSized = false;
      auto texture = handlerIt->second(inputConfig, skipTextureOptions,
                                       skipCache, framebufferSized);
      ...
      if (texture) {
        ...
        if (!skipCache)
          inputTextures.insert(
              make_pair(inputConfig.id, make_tuple(texture, framebufferSized)));
        else
          return *texture;
      } else {
        BOOST_LOG_TRIVIAL(error) << "failed loading " << inputConfig.type
                                 << " texture for input " << inputConfig.id;

        if (!skipCache)
          inputTextures.insert(
              make_pair(inputConfig.id, make_tuple(emptyTexture, false)));
        else
          return *emptyTexture;
      }
    } else {
      BOOST_LOG_TRIVIAL(error) << "unknown texture type " << inputConfig.type
                               << " for input " << inputConfig.id;
    }
  }
...
void texture_engine::ApplyTextureOptions(const InputConfig &inputConfig,
                                         gl::texture &texture) {
...
void texture_engine::RegisterHandler(const string &name, InputHandler handler) {
  if (name.empty())
