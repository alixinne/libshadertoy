...
private:
  ...
  std::shared_ptr<texture_engine> textureEngine;

  /// Aux buffers
  std::map<std::string, std::shared_ptr<toy_buffer>> buffers;
   ...
   */
  virtual std::shared_ptr<texture_engine> BuildTextureEngine();
   ...
   */
  virtual void
  LoadBufferSources(std::vector<std::pair<std::string, std::string>> &sources);
...
public:
	 ...
	 */
        inline texture_engine &GetTextureEngine() { return *textureEngine; }
	 ...
	 */
        void BuildBufferShader(const std::string &id, gl::shader &fs);
	 ...
	 */
        const GLchar *LoadShaderSource(
            const boost::filesystem::path &path) throw(std::runtime_error);
	 ...
	 */
        inline ShaderInputsType &GetState() { return state; }

        /**
	 * @brief      Gets a reference to the configuration objet for this context.
	 */
        inline context_config &GetConfig() { return config; }
	 ...
	 * @return
	 */
        std::vector<std::shared_ptr<BoundInputsBase>>
        GetBoundInputs(gl::program &program);
	 ...
	 *              buffer exists.
	 */
        std::shared_ptr<toy_buffer>
        GetBufferByName(const std::string &name = std::string());
};
