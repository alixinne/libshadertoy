 ...
 * for more GLSL types.
 */
struct shadertoy_EXPORT DynamicShaderInputsGLSLTypeVisitor
    : public boost::static_visitor<GLSLTypeInfo> {
  inline GLSLTypeInfo operator()(int) const {
    return std::make_tuple("int", "");
  }
  inline GLSLTypeInfo operator()(glm::ivec2) const {
    return std::make_tuple("ivec2", "");
  }
  inline GLSLTypeInfo operator()(glm::ivec3) const {
    return std::make_tuple("ivec3", "");
  }
  inline GLSLTypeInfo operator()(glm::ivec4) const {
    return std::make_tuple("ivec4", "");
  }

  inline GLSLTypeInfo operator()(float) const {
    return std::make_tuple("float", "");
  }
  inline GLSLTypeInfo operator()(glm::vec2) const {
    return std::make_tuple("vec2", "");
  }
  inline GLSLTypeInfo operator()(glm::vec3) const {
    return std::make_tuple("vec3", "");
  }
  inline GLSLTypeInfo operator()(glm::vec4) const {
    return std::make_tuple("vec4", "");
  }

  inline GLSLTypeInfo operator()(unsigned int) const {
    return std::make_tuple("unsigned int", "");
  }
  inline GLSLTypeInfo operator()(glm::uvec2) const {
    return std::make_tuple("uvec2", "");
  }
  inline GLSLTypeInfo operator()(glm::uvec3) const {
    return std::make_tuple("uvec3", "");
  }
  inline GLSLTypeInfo operator()(glm::uvec4) const {
    return std::make_tuple("uvec4", "");
  }
  ...
  GLSLTypeInfo operator()(std::array<T, N> &) const {
    return std::make_tuple(std::get<0>((*this)(T())),
                           std::string("[") + std::string(N) +
                               std::string("]") + std::get<1>((*this)(T())));
  }

  template <class T> GLSLTypeInfo operator()(std::shared_ptr<T> &) const {
    return (*this)(T());
  }
};
 ...
 */
class shadertoy_EXPORT DynamicShaderInputUniformSetter
 ...
 * @tparam Count       Number of instances of the type. Must be strictly
 * positive
 */
...
public:
  ...
  /**
   * @brief Appends the GLSL definition of this input to the given output
   * stream.
   *
...
public:
  ...
  void Add(const std::string &name, Args... args) {
    InputMap.insert(std::make_pair<std::string, VariantPtr>(
        std::string(name),
	 ...
	 */
        void Remove(const std::string &name) { InputMap.erase(name); }
	 ...
	 */
        void AppendDefinition(std::ostream &os) {
          ...
          for (auto &pair : InputMap) {
            auto type_info(
                boost::apply_visitor(GLSLTypeVisitor(), pair.second));
            os << "uniform " << std::get<0>(type_info) << " " << pair.first
               << std::get<1>(type_info) << ";" << std::endl;
          }
	}
	 ...
	 */
        std::map<std::string, gl::uniform_location>
        BindInputs(gl::program &program) {
          std::map<std::string, gl::uniform_location> result;

          for (auto &pair : InputMap) {
            result.insert(std::make_pair(
                pair.first, program.get_uniform_location(pair.first.c_str())));
          }
	...
	{
          return boost::apply_visitor(DynamicShaderInputUniformSetter(location),
                                      InputMap[name]);
        }
};
 ...
 */
template <class... Inputs> class shadertoy_EXPORT ShaderInputs {
public:
...
public:
  ...
  private:
    template <typename Input, typename Enable = void> struct Uniform {};
    ...
    struct Uniform<Input, typename std::enable_if<(Input::N > 0)>::type> {
      ...
      /**
       * @brief Applies the values of the given input to the associated
       * location.
       *
      ...
      bool set_value(std::shared_ptr<Input> &valptr) {
        return Location.set_value(
            valptr->Values().size(),
            static_cast<const typename Input::ValueType *>(
                valptr->Values().data()));
      }
    };
     ...
     * @tparam Type  Type of the shader input object in the parent state type
     * declaration
     * @return       true if the value has been set, false if the associated
     * uniform was inactive
     */
    template <size_t Index, class Type> bool set_value() {
     ...
     *
     * @tparam Indices Index range of the uniforms in the parent state type
     * declaration
     */
  ...
  public:
     ...
     */
    template <size_t... Indices>
    BoundInputs(StateType &state, gl::program &program,
                std::index_sequence<Indices...>)
        : State(state), Uniforms(Uniform<Inputs>(
                            std::get<Indices>(state.AllInputs), program)...) {}
	 ...
	 */
          template <size_t... Indices>
          void AppendDefinitions(std::ostream &os,
                                 std::index_sequence<Indices...>) {
            int _[] = {
                (std::get<Indices>(AllInputs)->AppendDefinition(os), 0)...};
            (void)_;
	}
	 ...
	 */
          template <typename Input,
                    typename = typename std::enable_if<(Input::N < 1)>::type>
          Input &V() {
	 ...
	 */
        ...
        typename Input::ValueType &V() {
          return std::get<0>(
              boost::get<std::shared_ptr<Input>>(InputMap[Input::Name])
                  ->Values());
        }
	 ...
	 */
        ...
        const typename Input::ValueType &V() const {
          return std::get<0>(boost::get<std::shared_ptr<Input>>(
                                 InputMap.find(Input::Name)->second)
                                 ->Values());
        }
	 ...
	 */
        ...
        const typename Input::ArrayType &V() const {
          return boost::get<std::shared_ptr<Input>>(
                     InputMap.find(Input::Name)->second)
              ->Values();
        }
	 ...
	 */
        ShaderInputs()
            : InputMap{(std::make_pair<std::string, VariantPtr>(
                  Inputs::Name, std::make_shared<Inputs>()))...},
              AllInputs((boost::get<std::shared_ptr<Inputs>>(
                  InputMap[Inputs::Name]))...) {}
};
}
...
typedef ShaderInputs<iResolution, iTime, iTimeDelta, iFrame, iFrameRate,
                     iChannelTime, iChannelResolution, iMouse, iChannel0,
                     iChannel1, iChannel2, iChannel3, iDate, iSampleRate>
    ShaderInputsType;
}
